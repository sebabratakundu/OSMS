<?php
	...
	class change_password{
		...
		static function check_password($old_password,$username){
			...
			self::$query = self::$db -> prepare("SELECT admin_password FROM admin_login WHERE admin_username = ?");
			...
			if(self::$result -> num_rows !=0){
				...
				self::$old_stored_pass = self::$old_stored_pass["admin_password"];
		...
		static function update_password($new_password,$username){
			...
			self::$query = self::$db -> prepare("UPDATE admin_login SET admin_password = ? WHERE admin_username = ?");
